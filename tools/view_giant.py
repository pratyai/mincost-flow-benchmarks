...
valcols = ['iters', 'sddm_calls_per_iter', 'time_s_per_arc_per_iter', 'fact_s_per_arc_per_iter']
solvers = ['tulip_approxchol', 'tulip_cmg', 'tulip_hypre']

df = pl.read_csv("../giant.csv").filter(pl.col('solver').is_in(solvers)).filter(pl.col('iters') < 200)
...
for vc in valcols:
    ...
    dfpbest = dfpbest.drop('best').melt(id_vars=['name'], variable_name='solver', value_name = vc)

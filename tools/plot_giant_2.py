...
problem_groups = [problems_1, problems_2, problems_3]
# problem_groups = [problems_1, problems_2 , problems_3, problems_4]
# problem_groups = [problems_5]
problems = sorted(list(itertools.chain(*problem_groups)))
...
AXIS = "arcs"
METRICS = ["iters", "time_us_per_arc_per_iter", "time_s"]
...
for METRIC in METRICS:
    for probs in problem_groups:
        all_markers = "^+d"[: len(probs)]
        all_markers = dict(zip(probs, all_markers))
        all_colours = ["#1f77b4", "#ff7f0e", "#2ca02c"][: len(probs)]
        ...
        x = np.linspace(tdf[:, AXIS].min(), tdf[:, AXIS].max(), num=100)
        for pcls in probs:

...
Base.@kwdef mutable struct Solver{Tv<:Number,Ti<:Integer} <: AbstractKKTSolver{Tv}
  ...
  # Laplacians related
  icc_solve::Function  # Solver for the SDDM system that yields `dy`
end

Tulip.KKT.backend(::Solver) = "Mcfp Tulip K1 MATLAB/ICC"
...
function Tulip.KKT.setup(
  ...
  Klow = ichol(Kperm);
  """
  @mget perm Kperm Klow

  local icc_solve =
    (b) -> begin
      @mput perm b Kperm Klow
      mat"""
      bperm = b(perm);
      [xperm, ~] = pcg(Kperm, bperm, 1e-8, 1000, Klow, Klow');
      x = zeros(n, 1);
      x(perm) = xperm;
      """
      @mget x
  ...
  return Solver{Tv,Ti}(m, n, A, θ, regP, regD, K, ξ, icc_solve)
end

...
Solves the Minimum Cost Flow Problem using Mosek via MathOptInterface.
"""
function solve(netw::Dimacs.McfpNet, config::Dict, float_type::Type{<:Number} = Float64)
    if float_type != Float64
        @warn "Mosek only supports Float64. Using Float64."
    end
    ...
    # Setup Optimizer
    # Use MOI.instantiate to get default bridges (important for VectorAffineFunction support)
    optimizer = MOI.instantiate(MosekTools.Optimizer; with_cache_type = Float64)
    ...
    for (key, val) in params
        if key != "verbose"
            # Mosek parameters are often strings like "MSK_DPAR_INTPNT_CO_TOL_PFEAS"
            # MOI.RawOptimizerAttribute handles this
    ...
    # 1. Bounds: 0 <= x[i] <= u[i]
    # Use Scalar constraints for Mosek compatibility
    # Batch add: MOI.add_constraints(optimizer, functions, sets)
    
    # x_funcs = [MOI.ScalarAffineFunction([MOI.ScalarAffineTerm(1.0, x[i])], 0.0) for i in 1:n_edges] # Or SingleVariable?
    # Mosek supports SingleVariable in Interval.
    
    # MOI.add_constraints(optimizer, x, [MOI.Interval(0.0, u[i]) for i in 1:n_edges])
    # x is Vector{VariableIndex}.
    MOI.add_constraints(optimizer, x, [MOI.Interval(0.0, u[i]) for i in 1:n_edges])

    # 2. Flow conservation: Ax = b
    # A is sparse. We need to construct ScalarAffineFunction for each row.
